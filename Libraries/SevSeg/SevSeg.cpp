...
//Refresh Display
/*******************************************************************************************/
//Given a string such as "Hello daniel", it will display the whole statement scrolling it 
//Each digit is scrolled every time the speed timed is elapsed, it must be used with keepscrolling on loop main function
void SevSeg::DisplayScroll(char* toDisplay)
{
	if (currentDisplay != toDisplay)
		{
...
void SevSeg::stopScrolling ()
{
	currentDisplay ="";
	isScrolling = false;
}
// keeps updating the display scrolling the statement set on displayscroll fn
// this needs to be called on every loop in the main program
void SevSeg::keepScrolling (int speed)
{
	if (strlen(currentDisplay) != 0)		
	 {
		if (iterator + numberOfDigits <= strlen(currentDisplay)) 
				{
				isScrolling = true;
				char* subbuff;
		  	subbuff =(char*)malloc(numberOfDigits);
				memcpy( subbuff, &currentDisplay[iterator],  numberOfDigits );
				//Serial.println(subbuff);
				DisplayString(subbuff, 0);
				free(subbuff);
				if (millis() - sevsegtimer >= speed) 
				{
						sevsegtimer = millis();
						iterator = iterator + 1;
				}
